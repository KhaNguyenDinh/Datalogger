				    	...
				    	if (strlen($folderName)!=4) {
							...
							foreach ($files as $file) {
							    ...
							    foreach ($lines as $line) {
							    	...
							    	if (isset($elements[4]) && date("Y-m-d H:i",strtotime($elements[3])) !== $time_file) {
							    		$mov = "NO"; break;
							    	}
							    }						
							    if ($mov =='YES') {
								    foreach ($lines as $line) {
								        ...
								        // Tách nội dung dựa trên tab
											...
											if ($existingRecord) {
											    // Xóa dữ liệu cũ
											    DB::table($folder_TXT)->where('id', $existingRecord->id)->delete();
											}
											// Thêm dữ liệu mới
											DB::table($folder_TXT)->insert([
											    'id_khuVuc' => $id_khuVuc,
											    'name' => $elements[0],
											    'number' => $elements[1],
											    'unit' => $elements[2],
